...
class TestResizeImage():  # pylint: disable=too-few-public-methods
    ...
    def __init__(self):
        ...
        self.resize_image(options.input_filename, options.output_filename,
                          image_max_width, image_max_height)

    @classmethod
    def resize_image(cls, input_filename, output_filename, max_width,
                     max_height):
        """Resize an image."""
        ...
        if orig_size[0] > max_width or orig_size[1] > max_height:
            print('Original size is ({}, {}) and max size is ({}, {})'.format
            ...
            print('Resized {} using ratio of {} since w_ratio = {} and '
                  'h_ratio = {}'.format(input_filename, scale_ratio, w_ratio,
                                        h_ratio))
            orig_image = orig_image.resize(((int(orig_size[0] * scale_ratio)),
                                            int(orig_size[1] * scale_ratio)),
                                           Image.ANTIALIAS)
            print('orig_image is now ({}, {})'.format
                  (int(orig_size[0] * scale_ratio),
                   int(orig_size[1] * scale_ratio)))
