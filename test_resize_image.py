...
class TestResizeImage(object):
    ...
    def __init__(self):
        ...
        parser.add_option("-i", "--input_filename", dest="input_filename",
                          help="Input filename. Default to empty.", default='')
    ...
    def resize_image(cls, input_filename, output_filename, max_width, max_height):
        ...
        if orig_size[0] > max_width or orig_size[1] > max_height:
            print 'Original size is (%d, %d) and max size is (%d, %d)' % \
            ...
            print 'Resized %s using ratio of %.2f since w_ratio = %.2f and h_ratio = %.2f' % \
                    (input_filename, scale_ratio, w_ratio, h_ratio)
            orig_image = orig_image.resize(((int(orig_size[0] * scale_ratio)),
                                            int(orig_size[1] * scale_ratio)),
                                           Image.ANTIALIAS)
            print 'orig_image is now (%d, %d)' % \
